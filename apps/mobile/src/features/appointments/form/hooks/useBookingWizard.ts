...
import { fetchPatientRelatives } from '@/features/patient-relatives/api/patient-relatives.service';
...
export function useBookingWizard(opts: {
  ...
  useEffect(() => {
    if (opts.mode !== 'patient' || !user) return;
  ...
  useEffect(() => {
    ...
    for (const [key, row] of Object.entries(map)) {
      const ref = profileDocRefFromRow(key, row);
      if (!ref) continue;
      const firstSvcId = documentsSlotRows[0]?.id ?? slotRows[0]?.id;
      if (!firstSvcId) continue;
      setFilesByService((prev) => {
        const svc = prev[firstSvcId] ?? {};
        if (svc[key]) return prev;
        return { ...prev, [firstSvcId]: { ...svc, [key]: ref } };
      });
    }
  }, [profileDocsQ.data, documentsSlotRows, slotRows]);
