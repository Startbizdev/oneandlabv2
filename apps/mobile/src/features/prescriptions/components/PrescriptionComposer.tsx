...
import { AlertTriangle, Download, FileOutput, Upload } from 'lucide-react-native';
...
export function PrescriptionComposer({
  ...
  const generateMut = useMutation({
    mutationFn: () => generatePrescriptionPdf(appointmentId, text),
    onSuccess: async (res) => {
      ...
      toast('PDF généré — partagez ou enregistrez sur le RDV', { type: 'success' });
    },
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      return savePrescriptionPdfToAppointment(appointmentId, pdfUri, pdfFileName);
    },
    onSuccess: async (res) => {
      if (!res.success) {
  ...
  const content = (
    <View style={styles.inner}>
      {hasExisting ? (
        <View style={styles.warn}>
          ...
          <Text style={styles.warnText}>
            Une ordonnance est déjà enregistrée. Téléchargez-la ou modifiez le texte ci-dessous
            pour en régénérer une nouvelle.
          </Text>
          <Button
            title="Télécharger l'ordonnance enregistrée"
            variant="outline"
            size="sm"
      ...
      <Input
        label="Prescription (médicaments, posologie, durée…)"
        value={text}
        onChangeText={setText}
        multiline
        placeholder="Ex. Doliprane 1000 mg — 1 cp × 3/jour pendant 5 jours…"
  ...
  return (
    <Card>
      <Text style={styles.cardTitle}>Créer une ordonnance</Text>
      {content}
    </Card>
  );
}
