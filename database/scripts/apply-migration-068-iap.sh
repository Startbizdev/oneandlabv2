#!/usr/bin/env bash
# Applique database/migrations/068_subscriptions_iap.sql (IAP Apple/Google)
#
# Usage :
#   ./database/scripts/apply-migration-068-iap.sh
#   ./database/scripts/apply-migration-068-iap.sh --local-only
#   ./database/scripts/apply-migration-068-iap.sh --remote-only
set -euo pipefail
...
SQL_FILE="$REPO_ROOT/database/migrations/068_subscriptions_iap.sql"
...
apply_local() {
    ...
    mysql --default-character-set=utf8mb4 \
      -h"${DB_HOST}" -P"${DB_PORT:-3306}" -u"$DB_USER" "$DB_NAME" < "$SQL_FILE" || {
        if mysql --default-character-set=utf8mb4 \
          ...
          -e "SHOW COLUMNS FROM subscriptions LIKE 'billing_source';" 2>/dev/null | grep -q billing_source; then
          echo "==> Migration 068 déjà appliquée (local)."
          return 0
        fi
        return 1
      }
  ...
  echo "==> OK local — colonnes IAP subscriptions appliquées."
}
...
apply_remote() {
  ...
  REMOTE_TMP="/tmp/oneandlab-068-iap.$$.$RANDOM.sql"
  ...
  echo "==> Application migration 068 sur MySQL prod..."
...
if mysql --default-character-set=utf8mb4 -h"\$H" -P"\$P" -u"\$U" "\$D" <"\$REMOTE_TMP"; then
  ...
  if mysql --default-character-set=utf8mb4 -h"\$H" -P"\$P" -u"\$U" "\$D" \
    -e "SHOW COLUMNS FROM subscriptions LIKE 'billing_source';" 2>/dev/null | grep -q billing_source; then
    rm -f "\$REMOTE_TMP"
    echo "Migration 068 déjà appliquée (prod)."
    exit 0
  fi
  rm -f "\$REMOTE_TMP"
  exit 1
...
REMOTE_EOF
  ...
  echo "==> OK distant — colonnes IAP subscriptions appliquées."
}
