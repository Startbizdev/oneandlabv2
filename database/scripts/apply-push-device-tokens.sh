#!/usr/bin/env bash
# Applique database/migrations/066_push_device_tokens.sql
#
# Usage :
#   ./database/scripts/apply-push-device-tokens.sh              # local + prod
#   ./database/scripts/apply-push-device-tokens.sh --local-only
#   ./database/scripts/apply-push-device-tokens.sh --remote-only
set -euo pipefail
...
SQL_FILE="$REPO_ROOT/database/migrations/066_push_device_tokens.sql"
...
apply_local() {
  ...
  echo "==> OK local — push_device_tokens appliqué."
}
...
apply_remote() {
  ...
  REMOTE_TMP="/tmp/oneandlab-066-push-tokens.$$.$RANDOM.sql"
  ...
  echo "==> Application migration 066 sur MySQL prod..."
...
REMOTE_EOF
  ...
  echo "==> OK distant — push_device_tokens appliqué."
}
