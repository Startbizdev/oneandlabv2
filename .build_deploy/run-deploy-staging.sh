...
echo "==> Etape 1/2 menuswipe..."
"$SCRIPT_DIR/buildscript.sh"

echo "==> Preparation build staging (contournement verrou Windows sur .output)..."
rm -rf "$BUILD_DIR"
mkdir -p "$BUILD_DIR"
cd "$SCRIPT_DIR/frontend"
tar --exclude=node_modules --exclude=.output --exclude=.nuxt --exclude=node_modules/.vite -cf - . | tar -C "$BUILD_DIR" -xf -
ln -s "$SCRIPT_DIR/frontend/node_modules" "$BUILD_DIR/node_modules"

echo "==> Build local (production) dans staging..."
cd "$BUILD_DIR"
