...
export function initPassageFormFromAppointment(apt: Record<string, unknown>): PassageFormSnapshot {
...
export function buildAppointmentPassageUpdateBody(
  ...
  const fd = { ...(apt.form_data as Record<string, unknown> | undefined) } as Record<string, unknown>;
  ...
  const scheduledAtRaw = String(apt.scheduled_at ?? '');
  const dateYmd = scheduledAtRaw.slice(0, 10) || new Date().toISOString().slice(0, 10);
  const scheduledAt =
    timeSlot === 'all_day'
      ? `${dateYmd} 08:00:00`
