...
export function useNursePassageWeb() {
  ...
  async function createSeries(input: NursePassageSeriesInput): Promise<NursePassageSeriesCreateResult | null> {
    saving.value = true;
    try {
      const res = await apiFetch<{ success: boolean; data?: NursePassageSeriesCreateResult; error?: string }>('/nurse/passages/series', {
        method: 'POST',
        body: input,
      });
  ...
  async function fetchSeries(id: string): Promise<NursePassageSeries | null> {
    const res = await apiFetch<{ success: boolean; data?: NursePassageSeries; error?: string }>(`/nurse/passages/series/${id}`);
  ...
  async function updateSeries(
    ...
    saving.value = true;
    try {
      const res = await apiFetch<{ success: boolean; data?: NursePassageSeriesCreateResult; error?: string }>(`/nurse/passages/series/${id}`, {
        method: 'PATCH',
        body: input,
      });
  ...
  async function materializeSeries(id: string): Promise<NursePassageSeriesCreateResult | null> {
    saving.value = true;
    try {
      const res = await apiFetch<{ success: boolean; data?: NursePassageSeriesCreateResult; error?: string }>(
