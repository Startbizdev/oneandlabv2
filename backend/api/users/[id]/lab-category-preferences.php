<?php
/**
 * Admin : GET/PUT des préférences de soins (catégories) d'un laboratoire par son ID.
 * GET /users/:id/lab-category-preferences
 * PUT /users/:id/lab-category-preferences body: { preferences: [{ category_id, is_enabled }] }
 */
...
// Vérifier que la table existe (migration 025)
...
if (!$profile || $profile['role'] !== 'lab') {
    http_response_code(404);
    echo json_encode(['success' => false, 'error' => 'Laboratoire introuvable']);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    try {
        $stmt = $db->prepare('
            SELECT lcp.id, lcp.category_id, lcp.is_enabled, cc.name, cc.description, cc.type, cc.icon, cc.image_url
