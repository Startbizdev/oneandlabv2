...
try {
    ...
    $bytes = $service->renderPngForProfile($profileId, $format, $raw);
    header('Content-Type: image/png');
    header('Content-Disposition: inline; filename="cary-qr-' . $profileId . '.png"');
    header('Cache-Control: private, max-age=60');
    echo $bytes;
} catch (Throwable $e) {
    error_log('admin/qr/png: ' . $e->getMessage());
    header('Content-Type: application/json');
