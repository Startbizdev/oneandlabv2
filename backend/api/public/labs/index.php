...
/**
 * API publique - Liste des laboratoires (comptes lab + sous-comptes avec profil public)
 * GET: Liste paginée pour pages SEO "Explorer les laboratoires"
...
try {
    ...
    $where = "role IN ('lab', 'subaccount') AND is_public_profile_enabled = TRUE AND public_slug IS NOT NULL AND public_slug != ''";
    $params = [];
    if ($city !== '') {
    ...
    if (count($rows) > 0) {
        ...
        $stmt = $db->prepare("SELECT reviewee_id, COUNT(*) as total_reviews, AVG(rating) as average_rating FROM reviews WHERE reviewee_id IN ($placeholders) AND reviewee_type IN ('lab', 'subaccount') AND is_visible = TRUE GROUP BY reviewee_id");
        $stmt->execute($ids);
    ...
    foreach ($rows as $row) {
        $name = 'Laboratoire';
        if (!empty($row['company_name_encrypted'] ?? '') && !empty($row['company_name_dek'] ?? '')) {
            $decrypted = $crypto->decryptField($row['company_name_encrypted'], $row['company_name_dek']);
            if (trim((string) $decrypted) !== '') {
                $name = trim((string) $decrypted);
            }
        }
        if ($name === 'Laboratoire') {
