...
echo "=== DEBUG APPOINTMENTS ZONE ===\n\n";

// 1. Vérifier la zone de couverture
echo "1. Zone de couverture de l'infirmier:\n";
...
// 2. Vérifier tous les rendez-vous pending
...
echo "   Total pending: " . count($allPending) . "\n";
foreach ($allPending as $apt) {
    echo "   - ID: {$apt['id']}, assigned_nurse_id: " . ($apt['assigned_nurse_id'] ?? 'NULL') . ", lat: " . ($apt['location_lat'] ?? 'NULL') . ", lng: " . ($apt['location_lng'] ?? 'NULL') . ", category_id: " . ($apt['category_id'] ?? 'NULL') . "\n";
}
echo "\n";

// 3. Vérifier les rendez-vous pending non assignés avec coordonnées
...
foreach ($pendingWithCoords as $apt) {
    ...
    echo "   - ID: {$apt['id']}, lat: {$aptLat}, lng: {$aptLng}, distance: " . round($distance, 2) . "km, inZone: " . ($inZone ? 'YES' : 'NO') . "\n";
}
echo "\n";

// 4. Vérifier les préférences de catégories
echo "4. Préférences de catégories de l'infirmier:\n";
...
foreach ($results as $result) {
    echo "   - ID: {$result['id']}, status: {$result['status']}, assigned_nurse_id: " . ($result['assigned_nurse_id'] ?? 'NULL') . "\n";
}
